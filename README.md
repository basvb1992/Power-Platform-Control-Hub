...
The `src/generated/` folder is **committed to source** — collaborators do not need to regenerate it. You still need to run `add-data-source` to create your own `power.config.json` connection wiring, but the TypeScript model/service files are already present.
...
- The `src/generated/` folder is **committed to source** because Dataverse-table files (`Aaduser*`, `Bots*`, `Ppa_resourcetombstones*`) require `--resource-name` flags and are not regenerated by a plain `add-data-source` run. Every collaborator still needs to run `add-data-source` to wire up their own connection IDs in `power.config.json`.
